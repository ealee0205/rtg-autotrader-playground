...
class AutoTrader(BaseAutoTrader):
    ...
    def on_hedge_filled_message(self, client_order_id: int, price: int, volume: int) -> None:
        ...
        if volume == 0:
            ...
            if self.hedge > self.position and (self.hedge * -1) + (self.hedge - self.position) < POSITION_LIMIT:
                self.hbid_id = next(self.order_ids)
                self.hbids.add(self.hbid_id)
                self.send_hedge_order(self.hbid_id, Side.BID, self.fut_ask, self.hedge - self.position)
            elif self.hedge < self.position and (self.hedge * -1) - (self.position - self.hedge) > -POSITION_LIMIT:
                self.hask_id = next(self.order_ids)
                self.hasks.add(self.hask_id)
                self.send_hedge_order(self.hask_id, Side.ASK, self.fut_bid, self.position - self.hedge)
